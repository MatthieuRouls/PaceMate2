...
export default function SessionCard({ session, onClick, showJoinButton = true }: SessionCardProps) {
  ...
  const cardContent = (
    <div
      className="group bg-white rounded-3xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer border-2 border-gray-100 hover:border-primary-500"
      onClick={onClick}
    >
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <h3 className="text-xl font-bold text-secondary-600 mb-3 line-clamp-2">{session.title}</h3>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary-500/10 text-primary-600">
          ...
          <div className="flex flex-col items-end gap-1.5">
            <span className="text-xs text-secondary-600/60 font-medium uppercase tracking-wider">Niveau</span>
            ...
            <span className="text-xs text-secondary-600/60 font-medium">{levelLabels[session.level_required - 1]}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 py-4 border-y border-gray-100">
          <div className="text-center">
            ...
            <div className="text-xs text-secondary-600/60 uppercase tracking-wider font-medium">Km</div>
          </div>
          <div className="text-center border-x border-gray-100">
            <div className="text-3xl font-bold text-secondary-600 mb-1">{session.target_pace || '5\'30"'}</div>
            <div className="text-xs text-secondary-600/60 uppercase tracking-wider font-medium">Allure</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-secondary-600 mb-1">{estimatedDuration}</div>
            <div className="text-xs text-secondary-600/60 uppercase tracking-wider font-medium">Min</div>
          </div>
        </div>

        {/* Time & Location */}
        <div className="space-y-3">
          <div className="flex items-center gap-3 text-secondary-600/70">
        ...
        {/* Description */}
        {session.description && (
          <p className="text-sm text-secondary-600/70 line-clamp-2 leading-relaxed">
            {session.description}
          </p>
        )}
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm">
              ...
              <div className="text-xs text-secondary-600/60">
          ...
          {isFull ? (
            <span className="px-5 py-2.5 rounded-xl bg-gray-100 text-gray-600 text-sm font-semibold">
              Complet
            </span>
            ...
            <Link
              ...
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary-500 to-primary-600 text-white text-sm font-semibold shadow-md hover:shadow-lg hover:scale-105 transition-all"
            >
              Rejoindre
            </Link>
          ) : showJoinButton && !onClick ? (
            <span className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary-500 to-primary-600 text-white text-sm font-semibold">
              Voir détails
            </span>
          ) : null}
        </div>
